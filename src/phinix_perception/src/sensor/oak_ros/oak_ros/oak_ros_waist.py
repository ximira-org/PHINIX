...
class OAKLaunch(Node):
    ...
    def __init__(self):
        ...
        with self.device:
            ...
            inCfg = self.device.getOutputQueue("stereo_cfg", 1, blocking=False)
            # Create a receive queue for each stream
            # qList = [device.getOutputQueue(stream, 8, blocking=False) for stream in streams]
            config = inCfg.get()

            qFrames = self.device.getOutputQueue(name="video", maxSize=1, blocking=False)
            qDet = self.device.getOutputQueue(name="nn", maxSize=1, blocking=False)
            qDisp = self.device.getOutputQueue(name="disparity", maxSize=1, blocking=False)
            qDepth = self.device.getOutputQueue(name="depth", maxSize=1, blocking=False)
            qleft = self.device.getOutputQueue(name="left", maxSize=1, blocking=False)
            qright = self.device.getOutputQueue(name="right", maxSize=1, blocking=False)
            detections = []
            fps = FPSHandler()
            text = TextHelper()
            # Output queues will be used to get the rgb frames and nn data from the outputs defined above
            qRgb = self.device.getOutputQueue(name="rgb", maxSize=1, blocking=False)
            ...
            while True:
                rgb_t_s = time.time()
                inRgb = qRgb.get()
                rgb_t_e = det_t_s = time.time()
                inDet = qDet.get()
                det_t_e = inframe_t_s = time.time()
                inFrame = qFrames.get()
                inframe_t_e = depth_t_s = time.time()
                inDepth = qDepth.get()
                depth_t_e = disp_t_s = time.time()
                inDisparity = qDisp.get()
                disp_t_e = time.time()
                # self.get_logger().info("########################################") 
                # self.get_logger().info("inrgb det frame time = {}".format(rgb_t_e - rgb_t_s))
                # self.get_logger().info("det time = {}".format(det_t_e - det_t_s))
                # self.get_logger().info("in frame time = {}".format(inframe_t_e - inframe_t_s))
                # self.get_logger().info("depth frame time = {}".format(depth_t_e - depth_t_s))
                # self.get_logger().info("disp frame time = {}".format(disp_t_e - disp_t_s))
                # self.get_logger().info("--------------------------")

                indet_pr_s = time.time()
                if inDet is not None:
                    detections = inDet.detections
                    counter += 1
                indet_pr_e = time.time()

                inrgb_pr_s = time.time()
                ...
                inrgb_pr_e = time.time()
            
                inframe_pr_s = time.time()
                ...
                # self.get_logger().info("det pr time = {}".format(indet_pr_e - indet_pr_s))
                # self.get_logger().info("inrgb pr time = {}".format(inrgb_pr_e - inrgb_pr_s))
                # self.get_logger().info("inframe pr time = {}".format(inframe_pr_e - inframe_pr_s))
                # self.get_logger().info("disp pr time = {}".format(disp_pr_e - disp_pr_s))
                # self.get_logger().info("depth pr time = {}".format(depth_pr_e - depth_pr_s))
                # self.get_logger().info("bbox pr time = {}".format(bbox_pr_e - bbox_pr_s))
                # self.get_logger().info("total time = {}".format(bbox_pr_e - rgb_t_s))
                # self.get_logger().info("#######################################")
